...
public class PostPublicationRepository : IPostPublicationRepository
{
    ...
    {
        var publishCutoff = utcNow.AddMinutes(-15);
        var recentSyncCutoff = utcNow.AddMinutes(-50);
        return await _context.PostPublications
            .Include(p => p.SocialAccount)
            .Where(p => p.Status == PublicationStatus.Published
                        && p.PublishedAt != null
                        && p.PublishedAt <= publishCutoff
                        && !string.IsNullOrEmpty(p.ExternalPostId))
            .Where(p => !p.Analytics.Any(a => a.CollectedAt >= recentSyncCutoff))
